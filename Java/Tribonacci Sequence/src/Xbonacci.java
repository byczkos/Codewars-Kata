...
//    recommend to any math enthusiast and for showing me this mathematical curiosity too with his usual
//    contagious passion :)]
...
//   === check the solution ===
    public static void main(String[] args) {
        ...
        xbonacci.tribonacci(new double[]{1.0, 1.0, 1.0}, 10);
    }
    ...
    public double[] tribonacci(double[] s, int n) {
        ...
        for (int i = 0; i < n; i++) {
            if (i < 3) {
                tribonacciArray[i] = s[i];
            } else {
                double sum = tribonacciArray[i - 3] + tribonacciArray[i - 2] + tribonacciArray[i - 1];
                tribonacciArray[i] = sum;
            }
        }
        for (int i = 0; i < tribonacciArray.length; i++) {
            System.out.println(tribonacciArray[i]);
        }
