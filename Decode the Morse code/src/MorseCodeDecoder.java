...
//    case-insensitive, traditionally capital letters are used. When the message is written in Morse code,
...
//    URL: https://www.codewars.com/kata/decode-the-morse-code/train/java
...
public class MorseCodeDecoder {
    ...
    public static String decode(String morseCode) {
        ...
        String[] morseCodeTable = morseCode.split(" ");
        for (int i = 0; i < morseCodeTable.length; i++) {
            System.out.println(morseCodeTable[i]);
        }
        for (int i = 0; i < morseCodeTable.length; i++) {
            if (morseCodeTable[i].isEmpty() || morseCodeTable[i] == null) {
                builder.append(" ");
            } else {
                builder.append(MorseCode.get(morseCodeTable[i]));
            }
        }
